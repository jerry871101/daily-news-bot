# 版本：v20.0 (究極防護版：加入 2000 字強制截斷機制，徹底杜絕超大亂碼塞爆 Token 限制)
import os          
import feedparser  
import time
import datetime    
...
def prepare_news_summaries(news_list, news_type="general"):
    ...
    for news in news_list:
        # 🛡️ 關鍵升級：強制截斷超大新聞亂碼！只取前 2000 字，徹底避免 Token 爆炸
        raw_summary = news.get('summary', '無提供內文簡介')
        safe_summary = raw_summary[:2000] 
        
        zh_title, ai_analysis = process_news_with_api(news.title, safe_summary, news_type)
